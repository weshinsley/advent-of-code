...
test <- function() {
  d <- parse("../inputs/d16-test.txt")
  useful <- d[d$valve == "AA" | d$rate > 0, c("valve", "rate")]
  shortest <- get_shortest(d, useful)
  
  stopifnot(part1(useful, shortest) == 1651)
  stopifnot(part2(useful, shortest) == 1707)
}

test()
d <- parse("../inputs/d16-input.txt")
useful <- d[d$valve == "AA" | d$rate > 0, c("valve", "rate")]
shortest <- get_shortest(d, useful)

part1(useful, shortest)
part2(useful, shortest)
