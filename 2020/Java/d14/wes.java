...
public class wes {
  ...
  void run() throws Exception {
    List<String> w = Files.readAllLines(new File("d14/test1_165.txt").toPath());
    stop_if_not(solve(w, false), 165, "Test 1");
    w = Files.readAllLines(new File("d14/test2_208.txt").toPath());
    stop_if_not(solve(w, true), 208, "Test 2");
    
    w = Files.readAllLines(new File("d14/wes-input.txt").toPath());
