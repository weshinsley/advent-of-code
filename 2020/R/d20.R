d <- readLines("../Java/d20/wes-input.txt")
...
recurse(tiles, rep(NA, 144), 1:144, 1)
options(digits=14)
...
for (i in 1:144) {
  x <- ((i - 1) %% 12)
  y <- as.integer((i - 1) / 12)
  cat("x=",x,"y=",y,"i=",i,"GR=",got_result[i],"\n")
  cat("yy=",(1+(y*8)),(8+(y*8))," xx=",(1+(x*8)):(8+(x*8)),"\n")
