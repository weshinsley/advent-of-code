...
vis <- function(d, matr, file = "wes-input.pg", px = 2000, py = 2000) {
...
test <- function() {
  d <- parse_input("../inputs/d25-test.txt")
  d$links$pzl <- d$links$pzl[d$links$pzl != "hfx"]
  d$links$cmg <- d$links$cmg[d$links$cmg != "bvb"]
  d$links$jqt <- d$links$jqt[d$links$jqt != "nvd"]
  d$links$ntq <- d$links$ntq[d$links$ntq != "jqt"]
  stopifnot(part1(d) == 54)
}

test()
d <- parse_input("../inputs/d25-input.txt")
