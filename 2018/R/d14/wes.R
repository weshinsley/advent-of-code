advent14a <- function(input) {
  ...
  message(paste(recipes[(input+1L):(input+10L)], collapse=""))
}
...
advent14b <- function(input) {
  ...
  while (TRUE) {
    ...
    if (used >= capacity) {
      ...
      message(sprintf("Capacity is now %d", capacity))
    }
...
system.time(advent14a(as.integer(input)))
system.time(message(advent14b(input)))
