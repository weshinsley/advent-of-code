...
drive <- function(map, carts, remove_crashes) {
  ...
  while (!done) {
    ...
    while (i <= nrow(carts)) {
      ...
      if (new_ch %in% cart_ch) {
        if (!remove_crashes) {
          message(sprintf("Crash at %d,%d\n", carts$x[i]-1, carts$y[i]-1))
          done <- TRUE
          break
        } else {
          crash_flag <- TRUE
  ...
  if (remove_crashes) {
    message(sprintf("Last cart at %d,%d\n",carts$x[1]-1, carts$y[1]-1))
  }
}

map <- lapply(readLines("wes-input.txt"), function(x) unlist(strsplit(x, NULL)))
carts <- init_carts(map)
drive(map, carts, FALSE)
drive(map, carts, TRUE)
