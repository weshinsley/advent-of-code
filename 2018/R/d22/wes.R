...
advent22a <- function(depth, tx, ty) {
  ...
  message(sprintf("Total risk to target: %d", sum(map[1:(ty + 1), 1:(tx+ 1 )])))
  map
}
...
input <- parse_input(readLines("wes-input.txt"))
map <- advent22a(input[[1]], input[[2]][1], input[[2]][2])
estimate <- advent22b(map, Inf, input[[2]][1]+2, input[[2]][2]+2, input[[2]][1], input[[2]][2])
message(sprintf("Esimate = %d", estimate))
advent22b(map, estimate, input[[2]][1]+SPLATTER, input[[2]][2]+SPLATTER, input[[2]][1], input[[2]][2])
