...
example <- data.frame(depth = c(0,1,4,6), range = c(3,2,4,4), stringsAsFactors = FALSE)
input <- read.csv("input.txt", sep = ":", stringsAsFactors = FALSE, header = FALSE)
names(input) <- c("depth","range")

test(advent13a(example),24)
advent13a(input)

test(advent13b(example),10)
advent13b(input)
