...
public class wes {
  ...
  void test() throws Exception {
    ArrayList<String> example = Utils.readLines("../R/22/example.txt");
    parse(example);
    Utils.test(solve(7, true), 5);
    parse(example);
    Utils.test(solve(70, true), 41);
    parse(example);
    Utils.test(solve(10000, true),  5587);
    parse(example);
    Utils.test(solve(100, false), 26);
    parse(example);
    Utils.test(solve(10000000, false), 2511944);
  }

  public static void main(String[] args) throws Exception {
    wes w = new wes();
    w.test();
    ArrayList<String> input = Utils.readLines("../R/22/input.txt");
    w.parse(input);
